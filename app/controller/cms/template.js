...
class TemplateController extends Controller {
	...
	*/
	async add() {
        ...
        const staticDir = this.config.static.dir
		const pub = path.join(staticDir, 'cms', 'default-1f8dda2a-6e9e-4762-a273-1bc138ec9ffa');
		const newpub = path.join(staticDir, 'cms', `${add.path}-${add.uuid}`);
	...
	*/
	async del() {
        ...
        const staticDir = this.config.static.dir
		const pub = path.join(staticDir, 'cms', `${info.path}-${info.uuid}`);
		await fs.remove(pub);
	...
	*/
	async listuse() {
        ...
        if (info.type === 'index') {
            await ctx.model.CmsTemplateList.update({ isu: false }, { where: { uuid: { [Op.ne]: uuid }, type: info.type, template_uuid: info.template_uuid } });
        }
		this.success(up);
	}
	/**
	...
	*/
	async export() {
        ...
        const staticDir = this.config.rundir;
		const tempmulu = path.join(staticDir, `${temp.path}-${temp.uuid}`);
		...
		const pub = path.join(this.config.static.dir, 'cms', `${temp.path}-${temp.uuid}`);
		const newpub = path.join(tempmulu, 'public', `${temp.path}-${temp.uuid}`);
		await fs.copy(pub, newpub);
		const file = path.join(staticDir, `${temp.path}-${temp.uuid}.tar`);
		await compressing.tar.compressDir(tempmulu, file, { prefix: '' });
        await ctx.downloader(file);
        fs.remove(file);
		fs.remove(tempmulu);
	}
	/**
	...
	*/
	async import() {
        ...
        const dirName = path.basename(file.filename, path.extname(file.filename)), mulu = path.join(this.config.rundir, dirName);
		try {
			await compressing.tar.uncompress(file.filepath, this.config.rundir);
			const temp = await fs.readJSON(path.join(mulu, 'temp.json'));
			const templist = await fs.readJSON(path.join(mulu, 'templist.json'));
			...
			const oldtemp = path.join(mulu, 'view', `${temp.path}-${temp.uuid}`);
			const newtemp = path.join(this.app.baseDir, 'app', 'view', 'cms', `${temp.path}-${temp.uuid}`);
			await fs.copy(oldtemp, newtemp);
			const pub = path.join(mulu, 'public', `${temp.path}-${temp.uuid}`);
			const newpub = path.join(this.config.static.dir, 'cms', `${temp.path}-${temp.uuid}`);
			await fs.copy(pub, newpub);
		} catch (e) {
	...
	*/
	async getTemplate() {
		...
		map.where.type = type;
        if (name) {
            map.where.name = { [Op.startsWith]: name };
        }
