...
class ConfigController extends Controller {
	...
	*/
	async edit() {
        ...
        // 需要更新远程配置本地保存文件,防止热重启服务器读取的还是老配置
        try {
            const res = fs.readFileSync(this.config.remoteConfigFile);
            const result = JSON.parse(ctx.helper.decipher(res.toString()));
            if (name == 'egg') {
                ctx.helper._.merge(result, data);
            } else {
                ctx.helper._.merge(result, { [name]: data });
            }
            await fs.writeFile(this.config.remoteConfigFile, ctx.helper.cipher(JSON.stringify(result)));
        } catch (err) {}
        ctx.app.messenger.sendToApp('reload-config', { name });
		this.success(data);
	}
	/**
	* @summary 配置管理
