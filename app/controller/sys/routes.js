...
class RoutesController extends Controller {
	...
	*/
	...
	async editPages() {
		...
		const mulu = path.join(this.app.baseDir, 'app', path.dirname(link));
		ctx.helper.mkdirsSync(mulu);
		const wenjian = path.join(mulu, path.basename(link));
		try {
			// const schemaData = new Uint8Array(Buffer.from(decodeURIComponent(data.schema)));
			// await fs.writeFile(wenjian, JSON.stringify(JSON.parse(decodeURIComponent(data.schema)), null, '\t'));
            await fs.writeFile(wenjian, JSON.stringify(JSON.parse(decodeURIComponent(data.schema)), null, 4));
