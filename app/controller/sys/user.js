	...
	*/
	async userEdit() {
        ...
        try {
            ctx.validate({
                username: { type: 'string', required: true, max: 15, trim: true },
                newpassword: { type: 'password', required: false, allowEmpty: true, min: 6, max: 20 },
                mobile: { type: 'mobile', required: false, allowEmpty: true },
                email: { type: 'email', required: false, allowEmpty: true }
            }, data);
        } catch (error) {
        ...
        const exist = await ctx.model.SysUser.findOne({ where: { username: data.username, uuid: { [Op.ne]: data.uuid } } });
        if (exist) {
            return this.fail('已存在相同的用户名，请重试！');
        }
