...
module.exports = app => {
	...
	const CmsDoc = app.model.define('cms_doc', {
		...
		title: { type: DataTypes.STRING, allowNull: false, comment: '内容标题' },
		classify_id: { type: DataTypes.INTEGER, allowNull: false, defaultValue: 0, comment: '分类ID' },
		...
		models_uuid: { type: DataTypes.UUID, allowNull: false, comment: '模型UUID' },
		type: { type: DataTypes.INTEGER, allowNull: false, defaultValue: 2, comment: '内容类型（1-目录，2-主题，3-段落）' },
		...
		view: { type: DataTypes.INTEGER.UNSIGNED, allowNull: false, defaultValue: 0, comment: '浏览量' },
		level: { type: DataTypes.INTEGER.UNSIGNED, allowNull: false, defaultValue: 0, comment: '优先级（越高排序越靠前）' },
		status: { type: DataTypes.INTEGER.UNSIGNED, allowNull: false, defaultValue: 1, comment: '数据状态（0-禁用，1-正常，2-待审核，3-草稿）' },
		template: { type: DataTypes.STRING, comment: '模板详情' },
		tags: { type: DataTypes.STRING, comment: '标签' },
		sort: { type: DataTypes.INTEGER, allowNull: false, defaultValue: 0, comment: '排序同级有效越小越靠前' },
