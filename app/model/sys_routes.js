...
module.exports = app => {
	...
	const SysRoutes = app.model.define('sys_routes', {
		...
		path: { type: DataTypes.STRING, allowNull: false, defaultValue: '', comment: '路由 URL 路径' },
		icon: { type: DataTypes.STRING, comment: '配置菜单的图标' },
		verb: { type: DataTypes.ENUM, values: ["head","options","get","put","post","patch","del","redirect","resources"], allowNull: false, defaultValue: 'get', comment: '用户触发动作，支持 get，post 等所有 HTTP 方法' },
		...
		class_uuid: { type: DataTypes.UUID, allowNull: false, comment: '关联classify的uuid' },
		...
		app: { type: DataTypes.STRING, allowNull: false, comment: '应用标识' },
