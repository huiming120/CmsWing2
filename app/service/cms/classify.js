...
class classifyService extends Service {
	...
	async breadcrumb(cid) {
		...
		// const n = await this.ctx.model.CmsClassify.findOne({ where: { id: { [Op.eq]: cid } } });
        const n = classifyList[cid];
		const barr = [];
		const no = {};
		no.name = n.title;
		no.url = `/cms/list/${n.name ? n.name : n.id}.html`;
        /*
		const nn = await this.ctx.model.SysNavigation.findOne({
			where: {
				[Op.or]: [
					{ url: `/cms/list/${n.name}.html` },
					{ url: `/cms/list/${n.id}.html` },
				]
			}
		});
		if (nn) {
		...
		while (n.pid !== 0) {
			// const nav = await this.ctx.model.CmsClassify.findOne({ where: { id: { [Op.eq]: n.pid } } });
            const nav = classifyList[n.pid];
			const obj = {};
			obj.name = nav.title;
			obj.url = `/cms/list/${nav.name ? nav.name : nav.id}.html`;
            /*
			const navn = await this.ctx.model.SysNavigation.findOne({
				where: {
					[Op.or]: [
						{ url: `/cms/list/${nav.name}.html` },
						{ url: `/cms/list/${nav.id}.html` },
					]
				}
			});
			if (navn) {
	...
	// 获取子栏目包括本栏目
	async getSubClassifyIds(pid, ids = [], useDB = true) {
        ids.push(Number(pid));
        if (useDB) {
            const items = await this.ctx.model.CmsClassify.findAll({ where: { pid } });
            for (const v of items) {
                await this.getSubClassifyIds(v.id, ids, useDB);
            }
        } else {
            const classifyList = await this.app.cache.get('classifyList');
            const classify = classifyList[pid];
            const childrenIds = classify.childrenIds
            if (childrenIds.length > 0) {
                for (const id of childrenIds) {
                    await this.getSubClassifyIds(id, ids, useDB);
                }
            }
        }
		return ids;
