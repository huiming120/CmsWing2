...
class serverService extends Service {
	async restart() {
        ...
        if (this.config.env == 'prod') {
            setTimeout(() => {
                console.log('restart server111');
                const child = child_process.spawn('bash /home/cmswing/restartserver.sh', [], { detached: true, stdio: 'ignore' });
                child.on('close', (code) => {
